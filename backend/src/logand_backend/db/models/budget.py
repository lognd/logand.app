...
class BudgetEntry(Base):
    ...
    corrects_entry_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("budget_entries.id", ondelete="RESTRICT"), nullable=True
    )
...
class BudgetEntryEvidence(Base):
    ...
    budget_entry_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("budget_entries.id", ondelete="CASCADE"), nullable=False
    )
